...
class PAL_FeatureExtraction(FeatureExtraction):
	feature_list = {}
	...
	def precompute(self, datasetdscr, windows):
		...
		self.sensor_list={}
		for s in datasetdscr.sensor_desc:
		    name=s['ItemName']
			self.sensor_list[name] = {'name': name}
            self.sensor_list[name]['index'] = datasetdscr.sensor_id_map_inverse[s['ItemId']]
	...
	def _conver_window2eventlist(self, window):
		...
		for i in range(0,window.shape[0]):
			...
			cur_data_dict = {
					'datetime': window[i,1],
					'sensor_id': window[i,0],
					'sensor_status': window[i,2],
					#'activity':lastACT
				}
	...
	def _calculate_window_feature(self, window):
		...
		window_size=min(self.max_windowsize,len(event_list))
		cur_row_id=window_size-1
		...
		for (key, feature) in self.feature_list.items():
			if feature.enabled:
				...
				if feature.per_sensor:
					...
					x[feature.index]=
						feature.get_feature_value(data_list = event_list,
												  cur_index=cur_row_id,
												  window_size=window_size,
												  sensor_info=self.sensor_list,
												  sensor_name=None)
